...
def csv_saver(save_path, a, b, c, d, i, name='anatomy'):
...
save_path = '/storage/homefs/ng22l920/Codes/DeepPyramid_IJCARS/TrainIDs_Dataset_paper/'
Cat30_dir = '//storage/homefs/ng22l920/Datasets/Cat3K_30Vids_Segmentation/30videos_FinalRevision/'
...
for i in range(len(folds)):
    ...
    fold_test_Cat3K = cases.copy()


    for k in range(len(folds[i])):
        print(f'fold_test_Cat3K: {fold_test_Cat3K}')
        print(f'folds_Cat3K[i]: {folds[i]}')  
        fold_test_Cat3K.remove(folds[i][k])  
    ...
    print(locals()['imgs_Cat3K_train_fold_' + str(i)])    
    ...
    for j in range(len(fold_test_Cat3K)):    
        for f in glob.glob(Cat30_dir + str(fold_test_Cat3K[j]) + '/' +image_dir +  '*.png'):
            locals()['imgs_Cat3K_test_fold_' + str(i)].append(f)    

        for f in glob.glob(Cat30_dir + str(fold_test_Cat3K[j]) +'/' + mask_dir +  '*.png'):
